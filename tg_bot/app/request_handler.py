    ...
    async def delete_offer(self, offer_id):
        async with self.session.delete(self.url + "/offers/delete/" + str(offer_id)) as res:
            ...
            if res.status == 200:
                ...
                return {"ans": 0}
    async def decr_quantity_offer(self, offer_id, decr):
        data = {'offer_id': offer_id, 'decr': int(decr)}
        async with self.session.post(self.url + "/offers/decr/quantity", json=data) as res:
            if res.status == 200:
                return {"ans": 1}
            else:
                return {"ans": 0}

    async def add_photo_offer(self, offer_id, photo):
        data = {"offer_id": offer_id, "photo": photo}
        async with self.session.post(self.url + "/offers/add/photo", json=data) as res:
            if res.status == 200:
                return {"ans": 1}
            else:
                return {"ans": 0}
