...
@app.post("/offers/decr/quantity")
async def add_offer_can_add_in_group(offer: schemas.OfferQuantityDecr):
    try:
        quantity = db.session.query(models.Offer).filter_by(
            offer_id=offer.offer_id).first()
        if quantity.quantity - offer.decr > 0:
            db.session.query(models.Offer).filter_by(
                offer_id=offer.offer_id
            ).update({
                'quantity': int(quantity.quantity) - offer.decr
            })
        else:
            db.session.query(models.Offer).filter_by(
                offer_id=offer.offer_id
            ...
            await add_offer_hidden(schemas.OfferHidden(offer_id=offer.offer_id, hidden=True))
        db.session.commit()
        return {"ans": "ok"}
    except Exception as e:
        return {
...
@app.post("/offers/add/photo")
async def add_offer_photo(offer: schemas.OfferPhoto):
    try:
        db.session.query(models.Offer).filter_by(
            offer_id=offer.offer_id
        ).update({'photo': offer.photo})
        db.session.commit()
        return {"ans": "ok"}
    except Exception as e:
        return {"error": e}